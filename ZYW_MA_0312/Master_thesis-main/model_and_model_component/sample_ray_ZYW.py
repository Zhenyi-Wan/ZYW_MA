...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        if self.rgb is not None:
            rgb_image = self.rgb.squeeze(0).cpu().numpy()
    ...
    def get_all(self):
        ret = {
            ...
            "rgb_grad": self.rgb_grad.cuda() if self.rgb_grad is not None else None,
        }
        return ret
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        if self.rgb_grad is not None:
            rgb_grad = self.rgb_grad[select_inds]
        else:
            rgb_grad = None
        ...
        ret = {
            ...
            "idx": self.idx.cuda() if self.idx is not None else None,
            "rgb_grad": rgb_grad.cuda() if rgb_grad is not None else None,
        }
        return ret
