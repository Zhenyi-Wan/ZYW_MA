...
def get_2d_boxes(nusc,
    ...
    vehicle_list = ['vehicle.bus.bendy', 'vehicle.bus.rigid', 'vehicle.car', 'vehicle.construction',
                    'vehicle.emergency.ambulance',
                    'vehicle.emergency.police', 'vehicle.trailer', 'vehicle.truck'
                    ]
    ...
    for ann_rec in ann_recs:
        ...
        if len(ann_rec['attribute_tokens']) > 0:
            ...
            if ann_rec[
                'category_name'] in vehicle_list and current_attr == 'vehicle.moving' or current_attr == 'vehicle.stopped':
...
def main():
    save_path = '/media/lingaoyuan/SATA/Dataset/NuScene/data/nuscenes/bbox'
    ...
    for scene_number in range(len(nusc.scene)):
        ...
        if len(coco_infos_arrays) > 0:
            ...
            save_name = f"bbox_{scene_name}.json"

            print(f"{save_name} has complete the bbox")

            bbox_path = os.path.join(save_path, save_name)
