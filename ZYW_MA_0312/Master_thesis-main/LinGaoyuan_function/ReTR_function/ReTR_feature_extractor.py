...
from LinGaoyuan_function.ReTR_function.ReTR_cnn2d import ConvBnReLU, ResidualBlock
...
class FPN_FeatureExtractor(nn.Module):
    ...
    def __init__(self, out_ch=16):
        ...
        'LinGaoyuan_operation_20240916: due to the dimension does not match, just use two layer for featrue extraction for test, the relevant layers should be changed'
        self.inner = nn.Conv2d(32, 64, 1, stride=1, padding=0, bias=True)
        ...
        'LinGaoyuan_operation_20240916: due to the dimension does not match, just use two layer for featrue extraction for test, the relevant layers should be changed'
        # self.output = nn.Conv2d(64, out_ch, 3, stride=1, padding=1)
        'LinGaoyuan_operation_20240919: change input dim to 96 to fit the new test: intra_feat = F.interpolate(fea3, scale_factor=2, mode="bilinear")'
        self.output = nn.Conv2d(96, out_ch, 3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        'LinGaoyuan_operation_20240916: due to the dimension does not match, just use two layer for featrue extraction for test'
        # intra_feat = F.interpolate(fea2, scale_factor=2, mode="bilinear") + self.inner(fea1)
        'LinGaoyuan_operation_20240919: test for the output from three layer ignore add the fea2'
        intra_feat = F.interpolate(fea3, scale_factor=2, mode="bilinear")
