...
def sample_prior_depth_perturb(ray_o, ray_d, depth_prior, depth_offset_ratio = 0.05, N_samples_d = 32, inv_uniform=False, det=False):
    ...
    batch_size = depth_prior.size(0)
    z_vals = torch.zeros((batch_size, N_samples_d)).cuda()
...
def render_rays(
    ...
    if args.sample_with_prior_depth is True and use_updated_prior_depth is True:
        ...
        z_vals_total = torch.cat((z_vals, z_vals_with_prior_depth), dim=-1)
    ...
    if args.use_retr_model is True:
        if args.use_retr_feature_extractor is True:
            if args.use_volume_feature is not True:
                ...
                rgb = model.net_coarse.forward_clip(pts, ray_batch, rgb_feat, z_vals, mask, ray_d, ray_diff, ret_alpha=ret_alpha, latent_code=latent_code)
                ...
                rgb = model.net_coarse.forward_retr_clip(pts, ray_batch, rgb_feat, z_vals, mask, ray_d, ray_diff, ret_alpha=ret_alpha, fea_volume=feature_volume, latent_code=latent_code)
        ...
        rgb = model.net_coarse.forward_clip(rgb_feat, ray_diff, mask, pts, ray_d, latent_code)
    ...
    'LinGaoyuan_operation_20240925: always set N_importance=0 when we test clip, so the following code is useless during the clip test process'
    if N_importance > 0:
