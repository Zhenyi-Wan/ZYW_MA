...
class ZYW_ReTR_model(nn.Module):
    def __init__(self, args, in_feat_ch=32, posenc_dim=3, viewenc_dim=3, ret_alpha=False, use_volume_feature = False, dim_clip_shape_code = 0):
        ...
        if self.use_volume_feature and self.args.use_retr_feature_extractor:
            ...
            self.metallic_occu_transformer = LocalFeatureTransformer(d_model=self.in_feat_ch * 2 + self.PE_d_hid, nhead=8,
                                                            layer_names=['self'],
                                                            attention='full')
            self.roughness_occu_transformer = LocalFeatureTransformer(d_model=self.in_feat_ch * 2 + self.PE_d_hid, nhead=8,
                                                            layer_names=['self'],
                                                            attention='full')
            self.albedo_occu_transformer = LocalFeatureTransformer(d_model=self.in_feat_ch * 2 + self.PE_d_hid, nhead=8,
                                                            layer_names=['self'],
                                                            attention='full')
            self.normals_occu_transformer = LocalFeatureTransformer(d_model=self.in_feat_ch * 2 + self.PE_d_hid, nhead=8,
                                                            layer_names=['self'],
                                                            attention='full')
            ...
            self.metallic_decoder = LocalFeatureTransformer(
                d_model=self.in_feat_ch * 2 + self.PE_d_hid + self.dim_clip_shape_code, nhead=1, layer_names=['cross'],
                attention='full')
            ...
            # Inorder to better understand, rename the ray transformer as decoder(we do not output radiance)
            self.metallic_decoder = LocalFeatureTransformer(
                d_model=self.in_feat_ch + self.PE_d_hid + self.dim_clip_shape_code, nhead=1, layer_names=['cross'],
                attention='full')
            self.roughness_decoder = LocalFeatureTransformer(
                d_model=self.in_feat_ch + self.PE_d_hid + self.dim_clip_shape_code, nhead=1, layer_names=['cross'],
                attention='full')
            ...
            self.MaterialMLP = nn.Sequential(
                nn.Linear(
                    self.in_feat_ch + self.PE_d_hid + self.dim_clip_appearance_code + self.dim_clip_shape_code, 32),
                nn.ReLU(inplace=True),
                nn.Linear(32, 16), nn.ReLU(inplace=True),
                nn.Linear(16, 1)
            )
    ...
    def order_posenc(self, z_vals):
        ...
        pe = torch.zeros(z_vals.shape[0],z_vals.shape[1], self.PE_d_hid).to(z_vals.device)# Zhenyi Wan [2025/3/14] (batch_size, length, 8)
        position = z_vals.unsqueeze(-1)# Zhenyi Wan [2025/3/14] (batch_size, length, 1)
        ...
        return pe# Zhenyi Wan [2025/3/14] (batch_size, length, 8)
